...
vec3 rotX(vec3 v, float theta) {
    v.yz = rot2(v.yz, theta);
    return v;
}
vec3 rotY(vec3 v, float theta) {
    v.xz = rot2(v.xz, theta);
    return v;
}
vec3 rotZ(vec3 v, float theta) {
    v.xy = rot2(v.xy, theta);
    return v;
}
// 回転ノイズ（勾配ノイズの関数に回転を適用しただけ）
float rotNoise21(vec2 p, float theta) {
    vec2 n = floor(p);
    vec2 f = fract(p);
    float[4] v;
    for (int j = 0; j < 2; j++) {
...
float rotNoise31(vec3 p, float theta) {
    vec3 n = floor(p);
    vec3 f = fract(p);
    float[8] v;
    for (int k = 0; k < 2; k++) {
